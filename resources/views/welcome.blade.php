...
<style>
/* ═══════════════════════════════════════════════════════════
   QUIZMIND AI — FULL LANDING PAGE CSS
═══════════════════════════════════════════════════════════ */
...
.lp-section--dark > * { max-width: 1120px; margin-left: auto; margin-right: auto; padding-left: 24px; padding-right: 24px; }

/* ══════════════════════════════════════
   GRID LAYOUTS
══════════════════════════════════════ */
.grid-2 { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 24px; margin-top: 48px; }
.grid-3 { display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 22px; margin-top: 48px; }
.grid-5 { display: grid; grid-template-columns: repeat(auto-fit, minmax(180px, 1fr)); gap: 18px; margin-top: 48px; }
...
.feature-icon { font-size: 30px; display: block; margin-bottom: 16px; }
.feature-title {
  font-family: 'Syne', sans-serif;
  font-size: 16px;
  font-weight: 700;
...
.pdf-title { font-family:'Syne',sans-serif; font-size:clamp(26px,3.5vw,42px); font-weight:800; margin-bottom:16px; }
.pdf-desc { font-size:16px; color:var(--text1); max-width:520px; margin:0 auto 48px; }
.pdf-demo-box {
  background: var(--surface);
  border: 1px solid var(--border);
  border-radius: var(--radius-lg);
...
.pdf-icon { font-size: 40px; animation: floatY 3s ease infinite; }
.pdf-filename { font-size: 14px; font-weight: 600; color: var(--pur2); }
.pdf-progress-track {
  width: 100%;
  height: 6px;
...
.pdf-success { font-size: 13px; font-weight: 600; color: #00dc82; opacity: 0; transition: opacity .5s ease; }
...
.step-desc { font-size:14px; color:var(--text2); line-height:1.7; max-width:280px; margin:0 auto; }
...
.tutor-name { font-weight: 600; font-size: 14px; color: var(--text0); }
.tutor-status { font-size: 11px; color: #00dc82; }
...
.chat-msg--ai .chat-msg-ico { background: rgba(124,92,252,0.2); }
.chat-msg--user .chat-msg-ico { background: rgba(0,212,255,0.15); }
.chat-bubble {
  max-width: 75%;
  padding: 11px 15px;
  border-radius: 14px;
...
.tutor-meta { font-size: 12px; color: var(--text2); }
...
.source-card:hover { border-color: rgba(124,92,252,0.5); transform: translateY(-5px); }
.source-badge {
...
.source-ico { font-size: 32px; margin-bottom: 12px; display: block; }
.source-t { font-size: 14px; font-weight: 600; color: var(--text0); margin-bottom: 6px; }
.source-d { font-size: 12px; color: var(--text2); line-height: 1.5; }
.prog-track {
  height: 5px;
  background: rgba(255,255,255,0.08);
...
.builder-head-title { font-family:'Syne',sans-serif; font-size:15px; font-weight:700; color:var(--text0); }
.q-block {
...
.q-label { font-size:10px; font-weight:700; letter-spacing:2px; color:var(--pur); margin-bottom:8px; }
.q-text { font-size:14px; font-weight:500; color:var(--text0); margin-bottom:14px; }
...
.opt--correct { border-color: rgba(0,212,255,0.4); background: rgba(0,212,255,0.08); color: var(--cyan); }
.opt--wrong   { border-color: rgba(255,80,80,0.3); background: rgba(255,80,80,0.06); color: #ff8888; }
.builder-footer { display:flex; gap:10px; margin-top:16px; flex-wrap:wrap; }
...
.mode-card:hover { transform: translateY(-6px); box-shadow: 0 20px 40px rgba(0,0,0,0.3); }
.mode-card--solo  { background: rgba(124,92,252,0.07); border-color: rgba(124,92,252,0.3); }
.mode-card--duo   { background: rgba(0,212,255,0.06); border-color: rgba(0,212,255,0.3); }
.mode-card--team  { background: rgba(0,220,130,0.06); border-color: rgba(0,220,130,0.3); }
.mode-card--school{ background: rgba(255,165,0,0.06); border-color: rgba(255,165,0,0.3); }
.mode-card--ai    { background: rgba(255,80,150,0.06); border-color: rgba(255,80,150,0.3); }
.mode-pill {
  position: absolute; top: 14px; right: 14px;
  font-size: 10px; font-weight: 700;
  padding: 3px 9px; border-radius: 20px;
}
.mode-card--solo  .mode-pill { background: rgba(124,92,252,0.2); color: var(--pur); }
.mode-card--duo   .mode-pill { background: rgba(0,212,255,0.15); color: var(--cyan); }
.mode-card--team  .mode-pill { background: rgba(0,220,130,0.15); color: #00dc82; }
.mode-card--school.mode-pill { background: rgba(255,165,0,0.15); color: #ffa500; }
.mode-card--ai    .mode-pill { background: rgba(255,80,150,0.15); color: #ff5096; }
.mode-card--school .mode-pill { background: rgba(255,165,0,0.15); color: #ffa500; }
.mode-ico { font-size: 28px; margin-bottom: 14px; display: block; }
.mode-title { font-family:'Syne',sans-serif; font-size:15px; font-weight:700; color:var(--text0); margin-bottom:8px; }
.mode-desc { font-size: 12px; color: var(--text2); line-height: 1.6; }
...
.live-badge {
  ...
  font-size: 11px; font-weight: 700; color: #00dc82;
}
.live-dot {
...
.vs-label { font-size: 24px; font-weight: 900; color: #ffa500; text-align: center; font-family:'Syne',sans-serif; }
.player-name { font-size: 15px; font-weight: 700; color: var(--text0); margin-bottom:4px; }
.player-xp { font-size: 11px; color: var(--text2); margin-bottom: 10px; }
.score-bar-wrap { height: 7px; border-radius: 4px; overflow: hidden; margin-bottom: 4px; }
...
@media(max-width: 640px) { .inst-layout { grid-template-columns: 1fr; } }
.inst-panel {
  background: var(--surface);
  border: 1px solid var(--border);
  border-radius: var(--radius-lg);
...
.inst-panel:last-child { margin-bottom: 0; }
.inst-panel-title {
  font-family:'Syne',sans-serif;
  font-size: 15px; font-weight: 700; color: var(--text0); margin-bottom: 4px;
}
.inst-panel-sub { font-size: 12px; color: var(--text2); margin-bottom: 20px; }
.inst-row {
...
.inst-row-name { font-size: 13px; font-weight: 500; color: var(--text0); }
.inst-row-sub { font-size: 11px; color: var(--text2); }
.status-dot {
  width: 9px; height: 9px; border-radius: 50%;
  margin-left: auto; flex-shrink: 0;
}
.status-dot--live { background: #00dc82; box-shadow: 0 0 8px #00dc82; animation: pulse 1.5s infinite; }
.status-dot--soon { background: #ffa500; }
...
.cfg-item span { font-size: 11px; color: var(--text2); display:block; margin-bottom: 3px; }
.cfg-item strong { font-size: 13px; color: var(--text0); font-weight: 600; }
.lb-row {
...
.lb-rank { font-weight: 700; color: #ffa500; width: 30px; }
.lb-name { flex: 1; color: var(--text1); }
.lb-score { font-weight: 600; color: var(--text0); }
...
.cert-logo { font-family:'Syne',sans-serif; font-size:12px; font-weight:800; color:var(--pur); letter-spacing:3px; text-transform:uppercase; margin-bottom:6px; }
.cert-headline { font-size: 13px; color: var(--text2); margin-bottom: 18px; }
.cert-issued-to { font-size: 12px; color: var(--text2); margin-bottom: 8px; }
.cert-name { font-family: 'Syne', sans-serif; font-size: 32px; font-weight: 800; color: var(--text0); margin-bottom: 14px; }
.cert-body { font-size: 14px; color: var(--text1); line-height: 1.75; margin-bottom: 20px; }
.cert-body strong { color: var(--text0); }
.cert-subject-pill {
  display: inline-block;
  padding: 7px 20px;
  ...
  border: 1px solid rgba(124,92,252,0.4);
  border-radius: 30px;
  font-size: 13px; font-weight: 600; color: #c4bcff;
  margin-bottom: 28px;
}
.cert-footer {
  display: flex;
...
.cert-verify { font-size: 10px; font-weight: 600; color: var(--pur); text-transform: uppercase; letter-spacing: 1px; margin-top: 3px; }
...
@media(max-width: 600px) {
  ...
  .grid-3, .grid-5, .modes-grid, .source-grid { grid-template-columns: 1fr 1fr; }
  .opts-grid { grid-template-columns: 1fr; }
  .cert-card { padding: 28px 20px; }
}
@media(max-width: 400px) {
  .grid-3, .grid-5, .modes-grid, .source-grid { grid-template-columns: 1fr; }
}
</style>
...
<section id="features" class="lp-section">
  ...
  <div class="grid-3">
    @php $features = [
      ['🤖','AI Quiz Generator','Generate MCQs instantly from any topic, PDF, or educational image using Groq AI.'],
...
<section class="pdf-section lp-section--dark" style="padding:100px 24px;text-align:center">
  <div style="max-width:1120px;margin:0 auto">
    ...
    <div class="pdf-demo-box reveal reveal-delay-2" id="pdfDemo">
      ...
      <div class="pdf-progress-track"><div class="pdf-progress-bar" id="pdfBar"></div></div>
...
<section id="ai-tutor" class="lp-section">
  <div class="text-center mb-section reveal">
    ...
    <p class="section-sub">Chat with AI to master any concept — then convert the entire conversation into an MCQ quiz and play instantly.</p>
  </div>
  <div class="tutor-chat-demo reveal reveal-delay-1">
    ...
    @php $msgs = [
      ['user','Explain Newton\'s Third Law with a real example.','🙋'],
      ['ai',  'For every action there is an equal and opposite reaction. When you push off a wall while skating, the wall pushes you back with equal force — propelling you forward.','🤖'],
      ['user','Give me a rocket example too?','🙋'],
      ['ai',  'A rocket expels hot gas downward — the equal reaction force pushes the rocket upward. Thrust equals the rate of momentum change of expelled gases.','🤖'],
    ]; @endphp
    @foreach($msgs as $i => $m)
    <div class="chat-msg chat-msg--{{ $m[0] }}" style="animation:fadeUp .4s {{ $i * 0.15 }}s ease both">
...
<section id="generate" class="lp-section lp-section--dark" style="padding:100px 24px">
  <div style="max-width:1120px;margin:0 auto">
    <div class="text-center mb-section reveal">
      ...
      <p class="section-sub">PDF, image, AI chat, topic, URL — or build manually. Transform anything into exam-ready MCQs.</p>
    </div>
    <div class="source-grid">
      @php $sources = [
        ['📄','PDF / Document', 'NCERT, textbooks, notes — any PDF becomes MCQs in seconds','AI', 'pdf'],
        ['🖼️','Image / Photo',  'Snap a textbook page, diagram, or handwritten notes','Vision AI','img'],
        ['💬','AI Chat Session','Convert your tutor conversation into a quiz instantly','NEW','chat'],
        ['✍️','Manual Builder', 'Write your own questions with custom options and answers', null, null],
        ['🌐','Topic / URL',    'Enter any topic or paste a web link — AI generates from it', null, null],
      ]; @endphp
      @foreach($sources as $i => $s)
      <div class="source-card reveal reveal-delay-{{ $i+1 }}" @if($s[4]) onclick="animateSource(this,'{{ $s[4] }}')" @endif>
        @if($s[3])<span class="source-badge">{{ $s[3] }}</span>@endif
        <span class="source-ico">{{ $s[0] }}</span>
        <div class="source-t">{{ $s[1] }}</div>
        <div class="source-d">{{ $s[2] }}</div>
        @if($s[4])<div class="prog-track" style="display:none"><div class="prog-bar"></div></div>@endif
      </div>
      @endforeach
    </div>
    ...
    <div class="mcq-builder reveal reveal-delay-2">
      ...
      @php $qs = [
        ['PHYSICS','What is the SI unit of electric current?',['✓ Ampere','✗ Volt','Ohm','Watt']],
        ['CHEMISTRY','Atomic number of Carbon is?',['✗ 8','✓ 6','12','14']],
      ]; @endphp
      @foreach($qs as $q)
      <div class="q-block">
        ...
        <div class="opts-grid">
          @foreach($q[2] as $opt)
          <div class="opt {{ Str::startsWith($opt,'✓') ? 'opt--correct' : (Str::startsWith($opt,'✗') ? 'opt--wrong' : '') }}">{{ $opt }}</div>
          @endforeach
        </div>
      </div>
      @endforeach
...
<section id="play" class="lp-section">
  ...
  <div class="modes-grid">
    @php $modes = [
      ['🧠','Solo Practice',   'Study at your own pace. AI tracks weak spots and auto-suggests next topics.','PRACTICE','solo'],
      ['⚔️','Friend Battle',   'Invite via code. Live 1v1 MCQ battle with real-time scores and penalties.','1v1','duo'],
      ['👥','Team vs Team',    'Form squads of 2–10. Combined scores decide the champion team.','GROUP','team'],
      ['🏫','School vs School','Institution-wide battles with full leaderboards and department breakdowns.','EPIC','school'],
      ['🤖','Battle vs AI',   'No partner? Face our adaptive AI rival that matches your exact skill level.','AI RIVAL','ai'],
    ]; @endphp
  ...
  <div class="live-match-card reveal reveal-delay-2">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
      ...
      <div class="live-badge"><div class="live-dot"></div> IN PROGRESS</div>
    </div>
    <div class="vs-grid">
      <div>
        ...
        <div class="score-bar-wrap" style="background:rgba(124,92,252,0.15)">
          <div class="score-bar-fill" style="width:72%;background:linear-gradient(90deg,var(--pur),var(--pur2))"></div>
        </div>
      ...
      <div style="text-align:right">
        ...
        <div class="score-bar-wrap" style="background:rgba(0,212,255,0.12)">
          <div class="score-bar-fill" style="width:60%;background:linear-gradient(90deg,var(--cyan2),var(--cyan));margin-left:auto"></div>
        </div>
        <div class="score-label" style="color:var(--cyan);text-align:right">60% · 6/10</div>
      </div>
    </div>
    <div style="padding-top:16px;border-top:1px solid var(--border2);font-size:12px;color:var(--text2);text-align:center">
...
<section id="how" class="lp-section lp-section--dark" style="padding:100px 24px">
  <div style="max-width:1120px;margin:0 auto">
    ...
    <div class="grid-3">
      @php $steps = [
        ['1','Sign Up &amp; Choose Role','Register as student, teacher, institution, or parent. Link to your school with a reference code.'],
        ['2','Generate or Pick a Quiz','Use AI to generate MCQs from topic, PDF, image, or chat — or build manually.'],
        ['3','Battle &amp; Improve','Play solo, invite friends, or host team battles. AI tracks performance and suggests improvements.'],
      ]; @endphp
...
<section id="certificates" class="lp-section lp-section--dark" style="padding:100px 24px">
  <div style="max-width:1120px;margin:0 auto">
    <div class="text-center mb-section reveal">
      ...
      <p class="section-sub">QuizMind AI issues verified digital certificates for tournament winners and skill milestones — sharable on LinkedIn, WhatsApp and Instagram.</p>
    </div>
...
(function initThree() {
  ...
  var W = window.innerWidth, H = el.parentElement.clientHeight || window.innerHeight;
  var renderer = new THREE.WebGLRenderer({ antialias: true, alpha: true });
  renderer.setSize(W, H);
  renderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
  renderer.setClearColor(0x000000, 0);
  el.appendChild(renderer.domElement);
  Object.assign(el.style, { position:'absolute', inset:'0', zIndex:'0', pointerEvents:'none' });
  ...
  var count = 700, pos = new Float32Array(count * 3);
  ...
  var pMat = new THREE.PointsMaterial({ color: 0x7C5CFC, size: 0.04, transparent: true, opacity: 0.7 });
  ...
  var t1 = new THREE.Mesh(
    ...
    new THREE.MeshBasicMaterial({ color: 0x7C5CFC, wireframe: true, transparent: true, opacity: 0.15 })
  );
  t1.position.set(3, -0.5, 0); scene.add(t1);
  ...
  t2.position.set(-3.5, 1, 0); scene.add(t2);
  ...
  ico.position.set(0, -2.5, -2); scene.add(ico);

  /* Octahedron */
  var oct = new THREE.Mesh(
    new THREE.OctahedronGeometry(0.5, 0),
    new THREE.MeshBasicMaterial({ color: 0x9d7fff, wireframe: true, transparent: true, opacity: 0.1 })
  );
  oct.position.set(-1.5, 2, -1); scene.add(oct);
  ...
  function animate() {
    ...
    t1.rotation.x = t; t1.rotation.y = t * 0.7;
    t2.rotation.x = -t * 0.8; t2.rotation.z = t * 0.5;
    ico.rotation.x = t * 0.4; ico.rotation.y = t * 0.6;
    oct.rotation.y = t * 0.5; oct.rotation.z = t * 0.3;
  ...
  window.addEventListener('resize', function() {
    var nW = window.innerWidth, nH = el.parentElement.clientHeight || window.innerHeight;
    camera.aspect = nW / nH;
...
(function initPdfDemo() {
  ...
  var obs = new IntersectionObserver(function(entries) {
    if (entries[0].isIntersecting && !played) {
      ...
      setTimeout(function() { suc.style.opacity = '1'; }, 1900);
      obs.disconnect();
    }
  }, { threshold: 0.4 });
  obs.observe(box);
...
function animateSource(card, key) {
  ...
  var msgs = { pdf:'✅ 12 MCQs generated from PDF!', img:'✅ 8 MCQs extracted from image!', chat:'✅ 10 MCQs from chat session!' };
  setTimeout(function() { desc.textContent = msgs[key] || '✅ Done!'; desc.style.color = '#00dc82'; }, 1700);
  setTimeout(function() {
    bar.style.transition = 'none'; bar.style.width = '0';
    setTimeout(function() { bar.style.transition = 'width 1.5s ease'; track.style.display = 'none'; }, 50);
    desc.textContent = orig; desc.style.color = '';
  }, 4000);
}
</script>
@endsection
