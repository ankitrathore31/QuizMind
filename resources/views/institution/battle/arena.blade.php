...
<style>
    ...
    .tb-badge{padding:3px 10px;border-radius:12px;font-size:.7rem;font-weight:800;flex-shrink:0;}
    .tb-badge.host    {background:rgba(255,165,0,.15);color:#ffa833;border:1px solid rgba(255,165,0,.3);}
    .tb-badge.observer{background:rgba(0,212,255,.15);color:var(--ice);border:1px solid rgba(0,212,255,.3);}
    ...
    /* ══════════════ SPLIT-SCREEN LAYOUT ══════════════ */
    #hostArena{display:flex;flex:1;overflow:hidden;flex-direction:column;}
    ...
    #rankBanner.show{display:flex;align-items:center;gap:6px;flex-wrap:wrap;}
    .rb-item{
    ...
    .rb-item.rank-1{background:rgba(255,215,0,.1);border-color:rgba(255,215,0,.3);color:#ffd700;}
    .rb-item.rank-2{background:rgba(192,192,192,.08);border-color:rgba(192,192,192,.25);color:#c0c0c0;}
    .rb-item.rank-3{background:rgba(205,127,50,.08);border-color:rgba(205,127,50,.25);color:#cd7f32;}
    .rb-score{font-family:var(--fh);font-size:1rem;font-weight:900;}
    .rb-sep{color:rgba(255,255,255,.15);font-size:.9rem;}

    #lanesWrap{display:flex;flex:1;overflow:hidden;}
    ...
    .inst-lane:last-child{border-right:none;}
    .inst-lane.rank-1{border-right-color:rgba(255,215,0,.35);}
    .inst-lane.rank-2{border-right-color:rgba(192,192,192,.25);}
    ...
    .lane-hdr.rank-1{border-bottom-color:rgba(255,215,0,.5);background:rgba(255,215,0,.04);}
    .lane-hdr.rank-2{border-bottom-color:rgba(192,192,192,.3);}
    .lane-hdr.rank-3{border-bottom-color:rgba(205,127,50,.25);}

    .lane-top{display:flex;align-items:center;gap:10px;margin-bottom:8px;}
    .lane-icon{
        width:36px;height:36px;border-radius:10px;display:flex;align-items:center;justify-content:center;
        font-size:.9rem;font-weight:900;color:#fff;flex-shrink:0;
    }
    .lane-icon.s1{background:linear-gradient(135deg,#7c5cfc,#5a3dd4);}
    .lane-icon.s2{background:linear-gradient(135deg,#00d4ff,#0099cc);}
    .lane-icon.s3{background:linear-gradient(135deg,#f5c842,#d4a520);}

    .lane-inst-name{font-weight:900;font-size:.9rem;flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
    .lane-rank-em{font-size:1.5rem;flex-shrink:0;}

    /* Big institution score */
    .lane-score{
        font-family:var(--fh);font-size:2.2rem;font-weight:900;line-height:1;text-align:center;
        margin-bottom:8px;
        background:var(--gradient);-webkit-background-clip:text;-webkit-text-fill-color:transparent;
    }

    /* Score bar — visual comparison */
    .lane-score-bar-wrap{height:4px;background:rgba(255,255,255,.06);border-radius:2px;overflow:hidden;margin-bottom:10px;}
    .lane-score-bar{height:100%;border-radius:2px;transition:width .6s ease;width:0%;}
    .s1 .lane-score-bar{background:linear-gradient(90deg,#7c5cfc,#5a3dd4);}
    .s2 .lane-score-bar{background:linear-gradient(90deg,#00d4ff,#0099cc);}
    .s3 .lane-score-bar{background:linear-gradient(90deg,#f5c842,#d4a520);}

    .lane-stats{display:flex;gap:6px;justify-content:center;flex-wrap:wrap;}
    .lstat{text-align:center;background:rgba(255,255,255,.04);border-radius:6px;padding:5px 8px;flex:1;min-width:44px;}
    .lstat .v{font-weight:800;font-size:.85rem;}
    .lstat .l{font-size:.58rem;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;}

    /* ── Student rows inside lane body ── */
    .lane-body{flex:1;overflow-y:auto;padding:6px;}
    .lane-body::-webkit-scrollbar{width:3px;}
    .lane-body::-webkit-scrollbar-thumb{background:rgba(255,255,255,.08);border-radius:2px;}

    .stu-row{
        display:flex;align-items:center;gap:8px;padding:7px 10px;
        border-radius:8px;margin-bottom:3px;transition:background .2s;border:1px solid transparent;
        animation:stuIn .2s ease;
    }
    @keyframes stuIn{from{opacity:0;transform:translateX(-6px)}to{opacity:1;transform:none}}
    .stu-row.top   {background:rgba(255,215,0,.06);border-color:rgba(255,215,0,.15);}
    .stu-av{
    ...
    .stu-rank{font-size:.6rem;font-weight:900;color:var(--muted);width:18px;text-align:center;flex-shrink:0;}
    .stu-name{flex:1;font-size:.76rem;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
    .stu-streak{font-size:.68rem;color:#ff6b35;flex-shrink:0;}
    .stu-pts{font-family:var(--fh);font-weight:900;font-size:.82rem;color:var(--accent);flex-shrink:0;}
    .stu-disq{font-size:.62rem;color:var(--red);flex-shrink:0;}
    .stu-correct{font-size:.62rem;color:var(--green);flex-shrink:0;}

    /* ── Host Q bar (bottom strip) ── */
    #hostQBar{
        display:none;align-items:center;gap:14px;
        padding:10px 18px;background:rgba(0,0,0,.35);
        border-top:1px solid rgba(255,255,255,.07);flex-shrink:0;
    }
    #hostQBar.show{display:flex;}
    .hq-tc{position:relative;width:46px;height:46px;flex-shrink:0;}
    .hq-tc svg{transform:rotate(-90deg);}
    .hqtc-bg{fill:none;stroke:rgba(255,255,255,.08);stroke-width:5;}
    .hqtc-fill{fill:none;stroke-width:5;stroke-linecap:round;stroke-dasharray:132;stroke-dashoffset:0;transition:stroke .4s;}
    .hq-num{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-family:var(--fh);font-size:.95rem;font-weight:900;}
    .hq-text{flex:1;font-size:.82rem;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
    .hq-meta{font-size:.7rem;color:var(--muted);margin-top:2px;}
    .hq-answered{font-size:.72rem;color:var(--muted);flex-shrink:0;text-align:right;}

    /* ── Wait/Fin panels ── */
    .wait-panel,.fin-panel{
        display:none;flex-direction:column;align-items:center;justify-content:center;
        flex:1;text-align:center;gap:14px;padding:40px 24px;
    }
    .wait-panel.show,.fin-panel.show{display:flex;}
    .big-icon{font-size:3.5rem;animation:floatLoop 3s ease-in-out infinite;}
    ...
    #cdOverlay{
        ...
        background:rgba(0,0,0,.88);flex-direction:column;align-items:center;justify-content:center;text-align:center;
    }
    ...
    <div id="hostArena">
        ...
        <div class="wait-panel show" id="waitPanel">
            ...
            <h2 style="font-family:var(--fh);font-weight:900;font-size:1.4rem">
                @if($isCreator) Watching the battle… @else Observing the battle… @endif
            </h2>
            <p style="color:var(--muted);font-size:.84rem">
                @if($isCreator)
                    The battle has started. You are the host — you watch, not play.
                @else
                    {{ optional(\App\Models\Institution::find(optional(\App\Models\Institution::where('user_id', Auth::id())->first())->id))->name ?? 'Your institution' }} is competing. Watch the live scores below.
                @endif
            </p>
            @if($isCreator)
            <a href="{{ route('institution.battle.setup-page', $battle->code) }}" class="btn btn-ghost" style="margin-top:4px">← Back to Lobby</a>
            @endif
        </div>
        ...
        <div id="lanesWrap" style="display:none">
            @php
                $instList  = \App\Models\Institution::whereIn('id', $battle->participating_institutions ?? [])->get();
                $laneIcons = ['s1','s2','s3'];
                $laneEmoji = ['🥇','🥈','🥉'];
            @endphp
            @foreach($instList as $li => $inst)
            <div class="inst-lane {{ $laneIcons[$li] ?? 's1' }}" id="lane-{{ $inst->id }}" data-inst="{{ $inst->id }}">
                <div class="lane-hdr" id="lhdr-{{ $inst->id }}">
                    <div class="lane-top">
                        <div class="lane-icon {{ $laneIcons[$li] ?? 's1' }}">{{ strtoupper(substr($inst->name,0,1)) }}</div>
                    ...
                    <div class="lane-score-bar-wrap"><div class="lane-score-bar" id="lbar-{{ $inst->id }}"></div></div>
                    <div class="lane-stats">
                        <div class="lstat"><div class="v" id="lstu-{{ $inst->id }}">0</div><div class="l">Players</div></div>
                        <div class="lstat"><div class="v" id="lcor-{{ $inst->id }}">0</div><div class="l">Correct</div></div>
                        <div class="lstat"><div class="v" id="lacc-{{ $inst->id }}">—</div><div class="l">Accuracy</div></div>
                        <div class="lstat"><div class="v" id="lavg-{{ $inst->id }}">0</div><div class="l">Avg Pts</div></div>
                    </div>
                </div>
                <div class="lane-body" id="lbody-{{ $inst->id }}">
                    <div style="text-align:center;padding:20px;color:var(--muted);font-size:.78rem">Waiting for scores…</div>
                </div>
            </div>
            @endforeach
        </div>
        ...
        <div id="hostQBar">
            <div class="hq-tc">
                <svg width="46" height="46" viewBox="0 0 60 60">
                    ...
                    <circle class="hqtc-fill" id="tcFill" cx="30" cy="30" r="21" stroke="var(--accent)"/>
                </svg>
...
const S = {
    ...
    tTick: null, pTick: null,
    running: false, done: false, wsOk: false, startHandled: false,
    maxScore: 1,
};
...
function drawTimer(t) {
    ...
    const fill = g('tcFill'), num = g('tcNum');
...
function updateLanes(scores) {
    ...
    scores.forEach(s => {
        ...
        if (!byInst[id]) byInst[id] = { name: s.institution_name, score: 0, correct: 0, students: [], total: 0 };
    ...
    if (g('hostAnswerCount')) {
        g('hostAnswerCount').innerHTML = `<div style="font-size:.7rem;color:var(--muted)">Q${S.qIdx+1} answered</div><div style="font-weight:900">${totalAnswered}/${totalPlayers}</div>`;
    }
...
function endBattle() {
    ...
    clearTimer(); stopPoll();
    g('waitPanel').classList.remove('show');
    const lw = g('lanesWrap'); if (lw) lw.style.display = 'none';
    const qb = g('hostQBar'); if (qb) qb.classList.remove('show');
    const rb = g('rankBanner'); if (rb) rb.classList.remove('show');
    const fp = g('finishedPanel'); if (fp) fp.classList.add('show');
...
function showCountdown(cb) {
    ...
    ov.classList.add('show'); let n = 3; num.textContent = n;
