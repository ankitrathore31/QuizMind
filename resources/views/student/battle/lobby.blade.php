...
<style>
/* ── Layout ─────────────────────────────────────────────── */
.lobby-wrap   { max-width:720px;margin:0 auto;padding:32px 16px; }

/* ── Room code display ──────────────────────────────────── */
.room-code-big {
    font-family:var(--fh);font-size:2.8rem;font-weight:900;
    letter-spacing:.15em;background:var(--gradient);
    -webkit-background-clip:text;-webkit-text-fill-color:transparent;
    display:inline-block;
}
.copy-wrap { display:flex;align-items:center;gap:10px;justify-content:center;margin-bottom:6px; }

/* ── Invite link box ────────────────────────────────────── */
.invite-box {
    background:rgba(124,92,252,.05);border:1px solid rgba(124,92,252,.18);
    border-radius:var(--radius-sm);padding:10px 14px;
    display:flex;align-items:center;gap:10px;margin-bottom:12px;
}
.invite-url  { flex:1;font-size:.76rem;color:var(--muted);word-break:break-all; }
.copy-btn    {
    background:rgba(124,92,252,.12);border:1px solid rgba(124,92,252,.25);
    color:var(--accent);padding:5px 12px;border-radius:7px;font-size:.75rem;
    font-weight:700;cursor:pointer;white-space:nowrap;transition:.2s;flex-shrink:0;
}
.copy-btn:hover { background:rgba(124,92,252,.22); }
.copy-btn.copied { background:rgba(0,227,150,.12);border-color:rgba(0,227,150,.3);color:#00e396; }

/* ── Player list ────────────────────────────────────────── */
.players-grid { display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:20px; }
...
.team-panel h3 {
    font-family:var(--fh);font-size:.84rem;font-weight:800;margin-bottom:12px;
...
.player-name  { font-size:.85rem;font-weight:600;flex:1; }
.host-badge   {
    font-size:.62rem;background:rgba(255,165,0,.15);color:orange;
...
.empty-slot   { color:var(--muted);font-size:.78rem;text-align:center;padding:16px; }

/* ── Start button ───────────────────────────────────────── */
.start-btn    {
    background:var(--gradient);color:#fff;border:none;padding:16px;
    border-radius:var(--radius-sm);font-weight:800;font-size:1rem;
    width:100%;cursor:pointer;font-family:var(--fh);transition:.2s;
    margin-bottom:10px;letter-spacing:.02em;
}
.start-btn:hover:not(:disabled) { opacity:.9;transform:translateY(-1px);box-shadow:0 6px 24px rgba(124,92,252,.35); }
.start-btn:disabled              { opacity:.4;cursor:not-allowed;transform:none; }

.wait-card    { background:var(--card);border:1px solid var(--border);border-radius:var(--radius);padding:28px;text-align:center; }

/* ── Live indicator ─────────────────────────────────────── */
.live-badge {
    display:inline-flex;align-items:center;gap:6px;
    background:rgba(34,197,94,.1);border:1px solid rgba(34,197,94,.25);
    color:#22c55e;padding:4px 10px;border-radius:20px;font-size:.72rem;font-weight:700;
}
.live-dot { width:7px;height:7px;border-radius:50%;background:#22c55e;animation:pulseDot 1.5s infinite; }

/* ── Timer countdown (before start) ────────────────────── */
.countdown-wrap { text-align:center;padding:20px 0; }
.countdown-num  { font-family:var(--fh);font-size:5rem;font-weight:900;line-height:1;
...
<div class="lobby-wrap anim-fade">
    ...
    <div style="text-align:center;margin-bottom:28px">
        ...
        <p class="text-muted" style="font-size:.76rem">Share this code or link with your friends</p>
    </div>

    {{-- Invite links --}}
    @if($room->mode === 'team')
        <div class="invite-box">
            <span style="font-size:.72rem;font-weight:700;color:var(--accent);white-space:nowrap">Team A:</span>
            <span class="invite-url" id="linkA">{{ $room->team_a_url }}</span>
            <button class="copy-btn" onclick="copyText('{{ $room->team_a_url }}', this)">📋 Copy</button>
        </div>
        <div class="invite-box">
            <span style="font-size:.72rem;font-weight:700;color:#00e396;white-space:nowrap">Team B:</span>
            <span class="invite-url" id="linkB">{{ $room->team_b_url }}</span>
            <button class="copy-btn" onclick="copyText('{{ $room->team_b_url }}', this)">📋 Copy</button>
        </div>
    @else
        <div class="invite-box">
            <span class="invite-url">{{ $room->invite_url }}</span>
    ...
    @if($room->mode === 'team')
        <div class="players-grid">
            <div class="team-panel">
                <h3>
                    <span style="background:rgba(124,92,252,.15);padding:2px 8px;border-radius:5px;font-size:.72rem">A</span>
                    {{ $room->team_a_name }}
                    ...
                    @forelse($room->participants->where('team','a') as $p)
                        <div class="player-row">
                            ...
                            <span class="player-name">{{ $p->user->name }}
                                @if($p->user_id===$room->host_id)<span class="host-badge">HOST</span>@endif
                            </span>
            ...
            <div class="team-panel">
                <h3>
                    <span style="background:rgba(0,227,150,.12);padding:2px 8px;border-radius:5px;font-size:.72rem">B</span>
                    {{ $room->team_b_name }}
    ...
    @if(Auth::id() === $room->host_id)
        <button class="start-btn" id="startBtn"
            {{ $room->participants->count() < 2 ? 'disabled' : '' }}
            onclick="startBattle()">
            ⚡ Start Battle Now!
        </button>
    ...
    <div id="countdownOverlay" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:9999;
...
// Polls every 3s so lobby always stays fresh even if WS fails
