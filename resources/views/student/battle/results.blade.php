...
@section('content')
    <style>
        ...
        .p1 .pod-block {
            height: 80px;
            background: linear-gradient(180deg, rgba(255, 215, 0, .15), rgba(255, 215, 0, .06));
        ...
        .p2 .pod-block {
            height: 56px;
            background: linear-gradient(180deg, rgba(192, 192, 192, .1), rgba(192, 192, 192, .04));
        ...
        .p3 .pod-block {
            height: 40px;
            background: linear-gradient(180deg, rgba(205, 127, 50, .1), rgba(205, 127, 50, .04));
        ...
        .ac-card.clean {
            border-color: rgba(0, 227, 150, .18);
        }
        ...
        @media(max-width:640px) {
            ...
            .lb-row .lb-cell:not(.lb-cell-pts) {
                display: none;
            }
    ...
    <div class="results-wrap">
        ...
        <div class="res-header">
            ...
            <div class="res-sub">Room {{ $room->code }} · {{ $room->participants->count() }} Players ·
                {{ $room->total_questions }} Questions</div>
            @if ($room->mode === 'team')
        ...
        @elseif($topPlayer)
            <div class="winner-banner">
                ...
                <div class="wn-name">
                    {{ $topPlayer->user->name }}
                    @if ($topPlayer->user_id === Auth::id())
                        <span style="font-size:.7em;color:var(--muted)">(You!)</span>
                    @endif
                </div>
                <div class="wn-team" style="font-family:var(--fm)">{{ $topPlayer->score }} pts ·
                    {{ $topPlayer->correct }}/{{ $totalQ }} correct</div>
            </div>
        @endif
            ...
            <div>
                ...
                <div class="my-card">
                    <div>
                        ...
                        <div class="mc-sub">Rank #{{ $myRank }} of {{ $sorted->count() }} · {{ $room->mode }}
                            mode</div>
                    </div>
                    <div class="mc-stat">
                    ...
                    <div class="mc-stat">
                        <div class="mc-val {{ $myAccuracy >= 70 ? 'green' : ($myAccuracy >= 40 ? 'gold' : 'red') }}">
                            {{ $myAccuracy }}%</div>
                    ...
                    @foreach ([1 => ['p2', '🥈'], 0 => ['p1', '🥇'], 2 => ['p3', '🥉']] as $i => [$cls, $medal])
                        @if ($sorted->has($i))
                            ...
                            <div class="podium-slot {{ $cls }}">
                                ...
                                <div class="pod-name">{{ $p->user->name }}@if ($p->user_id === Auth::id())
            ...
            <div class="lb-table">
                @foreach ($sorted as $i => $p)
                    ...
                    <div class="lb-row {{ $p->user_id === Auth::id() ? 'me' : '' }} {{ $p->disqualified ? 'disq' : '' }}"
                        ...
                        <div class="lb-info">
                            ...
                            <div>
                                ...
                                <div class="lb-tags">
                                    ...
                                    @endif
                                    @if ($room->mode === 'team' && $p->team)
                                        <span class="tag tag-{{ $p->team }}">{{ strtoupper($p->team) }}</span>
                                    @endif
                                    @if ($p->disqualified)
                                        <span class="tag tag-disq">DISQ</span>
                                    @endif
                                    @if (!$p->disqualified && $totalVios > 0)
                                        <span class="tag tag-vio">⚠️ {{ $totalVios }} VIO</span>
                                    @endif
                        ...
                        <div class="lb-cell {{ $pAccuracy >= 70 ? 'green' : ($pAccuracy >= 40 ? 'gold' : 'red') }}">
                            {{ $pAccuracy }}%</div>
                            ...
                            <div class="ac-header">
                                ...
                                <div style="min-width:0;flex:1">
                                    <div class="ac-name">
                                        {{ $p->user->name }}
                                        @if ($p->user_id === Auth::id())
                                            <span style="color:var(--accent);font-size:.7em">● You</span>
                                        @endif
                                    </div>
                                    <div class="ac-sub">
                                        @if ($room->mode === 'team')
                                            Team {{ strtoupper($p->team ?? '?') }} ·
                                        @endif
                                        {{ $pAccuracy }}% accuracy
                                    </div>
                                </div>
        ...
        @php
            ...
            $totalCorrect = \App\Models\BattleQuestionAnswer::where('room_id', $room->id)
        ...
        <div>
            ...
            <div class="stat-chips">
                ...
                @if ($room->anti_cheat)
                    ...
                    <div class="chip"
                        style="border-color: {{ $disqCount > 0 ? 'rgba(255,77,106,.25)' : 'rgba(0,227,150,.2)' }}">
                        <span class="chip-icon">🛡️</span>
                        <div>
                            <div class="chip-val" style="color: {{ $disqCount > 0 ? 'var(--red)' : 'var(--green)' }}">
                                {{ $disqCount }}</div>
        ...
        <div class="action-bar">
            ...
            @endif
            <a href="{{ route('student.battle.setup') }}?quizId={{ $room->quiz_id }}" class="btn btn-ghost">🆕 New
                Battle</a>
