...
.setup-modal::-webkit-scrollbar { width: 4px; }
.setup-modal::-webkit-scrollbar-thumb { background: rgba(124, 92, 252, 0.2); }
</style>
@endpush
@endif
    ...
    <div class="setup-modal" id="setupModalCard">
        ...
        <div class="step-content">
            ...
            <div class="step-pane active" id="step0">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" id="field_display_name" placeholder="How others will see you…" maxlength="60">
            ...
            <div class="step-pane" id="step1">
                <div class="form-group">
                    ...
                    <input type="text" class="form-input" id="field_school_name" placeholder="Your institution name…" maxlength="120">
            ...
            <div class="step-pane" id="step2">
                ...
                <div class="form-group">
                    <label class="form-label">Bio <span style="font-weight:400;text-transform:none;letter-spacing:0">(optional)</span></label>
                    <textarea class="form-input" id="field_bio" placeholder="Tell the QuizMind community a bit about yourself…" maxlength="300"></textarea>
                    <div class="text-right" style="font-size:.72rem;color:var(--muted);margin-top:4px" id="bioCounter">0 / 300</div>
                </div>
                <div style="background:rgba(124,92,252,0.06);border:1px solid rgba(124,92,252,0.15);border-radius:var(--radius-xs);padding:14px 16px">
        ...
        <div class="modal-footer">
            ...
            <div class="flex gap-10">
                <button class="btn btn-ghost btn-sm" id="prevBtn" style="display:none" onclick="modalPrev()">← Back</button>
...
<div class="page" style="max-width:1280px">
    ...
    <div class="hero-banner anim-fade" id="heroBanner" style="position:relative;min-height:180px;overflow:hidden">
        ...
        {{-- Three.js canvas — sits behind all content --}}
        ...
        {{-- CSS sphere — hidden by JS when Three.js loads, visible on no-JS --}}
        ...
        <div style="position:relative;z-index:1">
            ...
            <div class="banner-actions">
                ...
                <a href="#" class="btn btn-ghost">⚔️ Find Battle</a>
            </div>
        </div>
    ...
    <div class="dash-grid">
                ...
                @foreach ($recentActivity as $a)
                    <div class="activity-item">
                        ...
                        <div style="flex:1;min-width:0">
                            ...
                            <div class="act-meta">
                                {{ $a['time'] }} · <span class="act-xp">+{{ $a['xp'] }} XP</span>
                            </div>
                        </div>
        ...
        <div>
                        ...
                        @for ($d = 6; $d >= 0; $d--)
                            @php
                                $active = $student->streak_last_date &&
                                    ...
                                    $d <= $student->streak - 1;
                            @endphp
                            <div class="streak-dot {{ $active ? 'active' : '' }}" title="{{ now()->subDays($d)->format('D') }}"></div>
                        @endfor
                    </div>
                </div>
            </div>
            ...
            {{-- Leaderboard --}}
            <div class="card mb-16 reveal reveal-delay-1">
                <div class="section-hd">
            ...
            <div class="card reveal reveal-delay-2">
                ...
                @foreach ($upcoming as $u)
                    <div class="upcoming-item">
                        <div class="upc-icon">{{ $u['icon'] }}</div>
                        <div style="flex:1;min-width:0">
                            <div class="upc-name">{{ $u['name'] }}</div>
                            <div class="upc-time">{{ $u['time'] }}</div>
                        </div>
                        <div class="upc-action">Join →</div>
                    </div>
                @endforeach
            </div>
...
(function () {
    ...
    // Hide CSS sphere — Three.js takes over
    ...
    /* ── Rotating icosahedron orb (right side of banner) ── */
    ...
    const pl1 = new THREE.PointLight(0x7C5CFC, 3, 12);
    pl1.position.set(3, 2, 3);
    scene.add(pl1);
    const pl2 = new THREE.PointLight(0x00D4FF, 2, 10);
...
if (bioField) bioField.addEventListener('input', () => {
...
function modalNext() {
    ...
    if (currentStep === totalSteps - 1) { submitProfile(); return; }
...
function submitProfile() {
    ...
    selectedSubjects.forEach(s => fd.append('subjects_interest[]', s));
    ...
    .then(d => {
        if (d.success) {
            ...
            qmToast('success', '🎉 Profile saved! Welcome to QuizMind!');
        } else {
            qmToast('error', d.message || 'Something went wrong.');
            nextBtn.textContent = "🎉 Let's Go!";
            nextBtn.disabled    = false;
        }
    })
    .catch(() => {
        qmToast('error', 'Network error. Please try again.');
        nextBtn.textContent = "🎉 Let's Go!";
        nextBtn.disabled    = false;
    });
}
